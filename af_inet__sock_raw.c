...
unsigned char *fill_transport_layer_after_ip(unsigned char *packet, int lenpacket, u_int8_t protocol, short src_port, short dst_port)
{
    ...
    int len_layer4_header_included = lenpacket - sizeof (struct iphdr);
    ...
    update_tcpudp_sum(packet, lenpacket , protocol);
...
int main()
{
	int packet_socket = socket(AF_INET, SOCK_RAW, IPPROTO_RAW); // can't be used to received all IP packets according to man 7 raw: "An IPPROTO_RAW socket is send only"
        // BUG (man 7 raw): 'When the IP_HDRINCL option is set, datagrams will not be fragmented and  are  limited to the interface MTU.'
	...
	/* test reception */
	   ...
	   socket_address.sin_addr.s_addr = iph->daddr;

           if(sendto (packet_socket, packet, len_packet , 0, (struct sockaddr *) &socket_address, sizeof (socket_address)) < 0)
           {
                   perror("sendto");
		   break;
           }
